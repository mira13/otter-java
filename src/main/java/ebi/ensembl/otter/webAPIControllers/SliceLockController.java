...
public class SliceLockController {
	...
	@Autowired private SliceLockService service;

	 
	  @PostMapping("/")
	  public List<SliceLock> findGeneByIdInDb(@RequestBody String body) {
	  ...
	  @PostMapping("/2")
	  public List<SliceLock> findGeneByIdInDbF(@RequestBody String body) {
		JSONObject jObject = new JSONObject(body);
		  
	    return service
	    		.getSliceLocksHibernates(
	    				jObject.getString("csName"), 
	    				jObject.getString("csVersion"), 
	    				jObject.getString("name")
	    				);  
