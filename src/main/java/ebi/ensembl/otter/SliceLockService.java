...
public class SliceLockService {
	...
	@Autowired private CoordSystemRepository coordSystemRepository;
	@Autowired private SeqRegionRepository seqRegionRepository;
	@Autowired private SliceLockRepository sliceLockRepository;
	...
	public List<SliceLock> getSliceLocksHibernates(String csName, String csVersion, String name) {
		...
		if (!cs.isEmpty()) {
			...
			List<SeqRegion> seqRegion =  seqRegionRepository
					.findByNameAndCoordSystemId(name, Integer.valueOf(cs.get(0).getCoordSystemId() ));
			 
		    if (!seqRegion.isEmpty()) {
		       List<SliceLock> sliceLockList = sliceLockRepository.findBySeqRegionIdAndActiveNot(seqRegion.get(0).getSeqRegionId(), "free");
	...
	/* This method is questionable for now. It runs 90ms vs 150 in above query for ch17, 
	 * but here if we need to add repo, we should manually implement query 
	 * that also not a big deal as we add repos not so often, and implement query - is 3 min of dev	
	*/
